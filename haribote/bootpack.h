...
struct SHEET {
	unsigned char *buf;//buf:图层所描画内容的地址
		int bxsize, bysize, vx0, vy0, col_inv, height, flags;
		//bxsize*bysize表示图层大小,vx0,vy0表示图层位置坐标,col_inv透明色色号,height：图层高度,falgs：图层的各种设定信息
	struct SHTCTL *ctl;
	struct TASK *task;
};
struct SHTCTL {
	...
	int xsize, ysize, top;
	struct SHEET *sheets[MAX_SHEETS];//256个地址
...
/* mtask.c */
#define MAX_TASKS		1000	/* 最大任务 */
#define TASK_GDT0		3		/* TSS的GDT起始段号 */
