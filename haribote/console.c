...
void cmd_exit(struct CONSOLE *cons, int *fat)
{
	...
	io_cli();
	if (cons->sht != 0) {
		fifo32_put(fifo, cons->sht - shtctl->sheets0 + 768);	/* 76-1023 */
	} else {
...
/*根据命令行内容判断文件名并运行相应的应用程序，若找到文件则返回1，没有找到文件则返回0*/
...
int *hrb_api(int edi, int esi, int ebp, int esp, int ebx, int edx, int ecx, int eax)
{
	...
	if (edx == 1) {
		...
		sheet_updown(sht, shtctl->top); /*  将窗口图层高度指定为当前鼠标所在图层高度，鼠标 移到上层 */
		reg[7] = (int) sht;
	} else if (edx == 6) {
