...
int strtol(char *s, char **endp, int base);	/* 标准stdlib.h函数 */
...
void HariMain(void)
{
	...
	int w = 70, h = 30, t = 4, spd_x = 1, spd_y = 1;
	int win, i, j, lang = api_getlang(), xskip = 0;
	char s[30], *p, *q = 0, *r = 0;

	//---------------------------------------change-----------------------------
	int timer;						 //控制光标闪烁的定时器
	int cursor_x, cursor_y, cursor_c;//光标位置及颜色
	int cur_pos;					 //光标在文本中的显示位置
...
err:
	...
	/* 准备窗口 */
	win = api_openwin(winbuf, w * 8 + 16, h * 16 + 37, -1, "notepad");
	api_boxfilwin(win, 6, 27, w * 8 + 9, h * 16 + 30, 7);

	cursor_x = 7, cursor_y = 29, cursor_c = 0/*黑色*/;
	cur_pos = 0;
		...
		if (i == '4') {
			if(cursor_x >= 14){
				...
				for (;;) {
					...
					if (api_getkey(0) != '4') { /* 如果没有连续按下'4'则处理结束 */
						break;
					}
				}
			}
...
{
	int i;
	api_boxfilwin(win + 1, 6, 29, w * 8 + 9, h * 16 + 30, 7);
	for (i = 0; i < h; i++) {
		p = lineview(win, w, i * 16 + 29, xskip, p, tab, lang);
	}
	api_refreshwin(win, 6, 29, w * 8 + 9, h * 16 + 30);
	return;
}
